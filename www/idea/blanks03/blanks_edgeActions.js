...
var Composition = Edge.Composition, Symbol = Edge.Symbol; // aliases for commonly used Edge classes
   ...
   (function(symbolName) {
       ...
       Symbol.bindSymbolAction(compId, symbolName, "creationComplete", function (sym, e) {
           ...
           function init() {
               sym.getSymbol('Symbol_p').$('p').draggable({opacity: 0.5, revert: on_revert, start: all_start, drag: all_drag});
               sym.getSymbol('Symbol_q').$('q').draggable({opacity: 0.5, revert: on_revert, start: all_start, drag: all_drag});
               sym.getSymbol('Symbol_d').$('t').draggable({opacity: 0.5, revert: on_revert, start: all_start, drag: all_drag});
               sym.getSymbol('Symbol_f').$('f').draggable({opacity: 0.5, revert: on_revert, start: all_start, drag: all_drag});
               sym.$('target_frame').droppable({
                   accept: sym.getSymbol('Symbol_d').$('t'),
                   drop: function () {
                       sym.getSymbol('Symbol_d').stop();
                       if (Utils.is_ios()) {
                           $('#yes_audio')[0].play();
                       } else if (Utils.is_android()) {
